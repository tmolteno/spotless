...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Calibrate the tart telescope from API using spotless.',
    ...
    for d in calib_info['data']:
        vis_json, src_json = d
        cv, ts, src_list = load_data_from_json(vis_json, src_json,
    ...
    s = calc_score(init_parameters, config, measurements,
                   window_deg, original_positions, update=False, show=False)

    def f(param):
        lambda param: calc_score(param, config, measurements,
                                 window_deg, original_positions,
    ...
    ret = optimize.minimize(f, init_parameters, method='L-BFGS-B',
                            bounds=bounds,
    ...
    fname = f"{output_directory}/spotless_opt.json"
    ...
    fname = f"{output_directory}/spotless_history.json"
    with open(fname, 'w') as fp:
